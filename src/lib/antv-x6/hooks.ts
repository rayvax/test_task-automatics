...
import { diffCells, patch, provideId } from './utils';
...
export const useGraphState = (initState: GraphState = {}) => {
  ...
  // Node changes may cause edge changes
  ...
  const setNodes = (_nodes: Node.Metadata[]) =>
    returnPromise(() => _setNodes(_nodes.map(provideId)));
  const setEdges = (_edges: Edge.Metadata[]) =>
    returnPromise(() => _setEdges(_edges.map(provideId)));
